...
def train_graphs(args: EasyDict, task_specific: str, metric_callback, measure_oversmoothing=False) -> tuple:
    ...
    forbenius_energy = trainer.callback_metrics.get("val_forbenius_energy", None)
    singular_energy  = trainer.callback_metrics.get("val_singular_energy", None)
    ...
    # forbenius_energy = trainer.callback_metrics.get("forbenius_grad_ratio", None).clone()
    # singular_energy = trainer.callback_metrics.get("singular_grad_ratio", None).clone()
    # Test model.
    test_results = trainer.test(model, test_loader, verbose=False)
    test_acc_model = test_results[0]['test_acc'] * 100
    return test_acc_model, forbenius_energy, singular_energy
...
def main():
    ...
    for current_depth in range(min_radius, max_radius):
        ...
        for repeat_idx in range(repeats):
            ...
            test_acc, forbenius_energy, singular_energy = train_graphs(args, task_specific, metric_callback, measure_oversmoothing=False)
        ...
        energy_results[current_depth] = forbenius_energy, singular_energy # Store energy for each radius
